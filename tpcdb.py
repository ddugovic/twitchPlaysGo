...
class conn():
    def __init__(self):
        self.conn = sqlite3.connect(
            "tpp.db", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
    ...
    def new_account(self, username, points=0):
        self.c.execute('INSERT INTO accounts(name, points, daily) VALUES (?, ?, ?)',
                       (username, int(points), datetime.datetime(2000, 1, 1, 1, 1, 1)))
        self.conn.commit()

    def game_end(self, result, level, voters, text):
        self.c.execute(
            'INSERT INTO games(result, level, voters) VALUES (?, ?, ?)', (result, level, voters))
        self.conn.commit()
        gameno = self.c.execute(
            'select seq from sqlite_sequence where name="games"').fetchall()[0][0]
    ...
    def get_game(self, gameid):
        ...
        try:
            temp = self.c.execute(
                'select * from games where no = ?', (gameid,)).fetchall()[0]
    ...
    def change_points(self, username, points):
        ...
        if self.get_points(username) + points < 0:
            return False
        self.c.execute(
            'UPDATE accounts set points = points + ? where name = ?', (int(points), username))
        self.conn.commit()
        return True
    ...
    def level_up(self, username):
        try:
            self.c.execute(
                'UPDATE accounts set level = level + 1 where name = ?', (username,))
        except:
            self.new_account(username)
            self.c.execute(
                'UPDATE accounts set level = level + 1 where name = ?', (username,))
    ...
    def set_level(self, level):
        self.c.execute(
            'update record set val = ? where field = "level"', (level,))
        self.conn.commit()

    def new_game(self):
        self.c.execute('delete from current')
        self.c.execute(
            'replace into record select * from next where field = "level"')
    ...
    def add_game_param(self, name, value, replace=False):
        try:
            self.c.execute(
                'INSERT INTO next(field, val) VALUES (?, ?)', (name, value))
        except:
            if replace:
                self.c.execute(
                    'UPDATE next set val = ? where field = ?', (value, name))
            else:
                return False
        self.conn.commit()
        return True
    ...
    def challenge(self, challenger, victim, amount):
        try:
            result = self.c.execute(
                'select timestamp from challenges where victim = ?', (victim,)).fetchall()[0][0]
        ...
        try:
            result = self.c.execute(
                'select timestamp, victim from challenges where challenger = ?', (victim,)).fetchall()[0]
            ...
            pass
        try:
            self.c.execute('insert into challenges(challenger,victim,amount,timestamp) Values (?, ?, ?, ?)',
                           (challenger, victim, amount, datetime.datetime.now()))
            self.conn.commit()
            return True
        except:
            return False
    ...
    def accept_challenge(self, victim):
        try:
            result = self.c.execute(
                'select challenger,victim,amount,timestamp from challenges where victim = ?', (victim,)).fetchall()[0]
    ...
    def get_daily_status(self, user):
        ...
        try:
            result = self.c.execute(
                'select daily from accounts where name = ?', (user,)).fetchall()[0][0]
    ...
    def reset_account_date(self, user):
        self.c.execute("update accounts set daily=? where name=?",
    ...
    def add_vip_points(self, user, amount):
        ...
        self.c.execute(
            "update accounts set vip=vip+? where name=?", (amount, user))
        self.conn.commit()
