...
class main(FloatLayout):
    def __init__(self, **kwargs):
        ...
        self.render = kiImage(pos=(-285, -85), height=900,
                              allow_stretch=True, size_hint_y=None)
        ...
        self.handicap_remaining = self.handicap_remaining = int(
            self.handicap / 19 * self.boardsize)
        ...
        self.info = Label(text=self.info_text, size_hint_y=1, size_hint_x=1,
                          markup=True, text_size=(545, 200), pos=(370, -15), valign="top")
        self.add_widget(self.info)

        self.move_options = Label(text=self.moves_string, markup=True, text_size=(
            545, 500), pos=(362, -445), valign="top")
        self.add_widget(self.move_options)

        self.set_legal_moves()
        self.update_history(reset=True)

        self.thinking_label = Label(text=self.format_text(
            "Thinking...", font_size=45), markup=True, text_size=(1260, 200), pos=(73500, 160), valign="top")
    ...
    def update_info(self, dt=0, text=None, hold=False):
        ...
        if text is None:
            ...
            if not c is None and "challenger" in c:
                ...
                self.info_text = "Opponent: Katago is %s giving %d extra stones" % (
                    opp_color, int(self.handicap / 19 * self.boardsize))
                self.info_text += "\nEval: {0}".format(self.board_evaluation)

            self.info_text += ", Game %d, W:%d, L:%d" % (
                self.round, self.record[0], self.record[1])
    ...
    def update_board(self):
        image = self.renderer.draw(self.fish.listStones("b"), self.fish.listStones(
            "w"), lastmove=self.lastmove, size=self.boardsize)
        data = BytesIO()
    ...
    def end_game(self, result):
        ...
        if result == "w":
            ...
            self.update_info(
                text="Twitch chat won, %d points awarded to participants" % payout, hold=True)
        ...
        if not c is None and "challenger" in c:
            ...
            self.handicap_remaining = int(
                self.handicap / 19 * self.boardsize) - 1
        if self.handicap_remaining > 0:
    ...
    def set_legal_moves(self, end=False):
        ...
        self.moves_string = self.format_text(
            "VIP leaderboard, use !vip to climb", font_size=26)
        temp = ""
        for i in range(5):
            temp += "\n%d. %s - %d points" % (vips[i]
                                              [2], vips[i][0], vips[i][1])
    ...
    def tally(self, dt):
        ...
        pyplot.figure(figsize=(3, 3))
        pyplot.bar(y, quantity, align='center',
                   alpha=0.5, width=1.0, color="black")
            ...
            if ctx.author.name == c["challenger"]:
                ...
                if processed in moves and not (ctx.author.name in votes):
                    ...
                    if lock.value and str(ctx.channel) == secrets['DEFAULT']['channel'][1:]:
                        await ws.send_privmsg("#%s" % ctx.channel, f"/me Inputs locked on this channel, visit https://twitch.tv/%s to play." % visiting.value)
                        return
    ...
    if len(params) > 0:
        ...
        if db.change_points(ctx.author.name, -delta):
            if random.choice([True, False]):
                ...
                await ws.send_privmsg("#%s" % ctx.channel, f"/me VoteYea %s wagered %d points and won, now they have %d points VoteYea" % (ctx.author.name, delta, db.get_points(ctx.author.name)))
            else:
...
async def command_rob(ctx):
    ...
    if result == 0:
        ...
        await ws.send_privmsg("#%s" % ctx.channel, f"/me %s got caught trying to rob a bank. SabaPing They had to pay %d points in bail." % (ctx.author.name, delta))
...
async def command_roll(ctx):
    ...
    if len(params) > 0:
        ...
        if db.change_points(ctx.author.name, -delta):
            ...
            if result <= 60:
                await ws.send_privmsg("#%s" % ctx.channel, f"/me VoteNay %s wagered %d points and rolled %d. Better luck next time VoteNay" % (ctx.author.name, delta, result))
            elif result <= 90:
                db.change_points(ctx.author.name, delta * 2)
                await ws.send_privmsg("#%s" % ctx.channel, f"/me VoteYea %s rolled %d. They won %d points for rolling above 60, now they have %d points VoteYea" % (ctx.author.name, result, delta * 2, db.get_points(ctx.author.name)))
            elif result <= 95:
                db.change_points(ctx.author.name, delta * 3)
                await ws.send_privmsg("#%s" % ctx.channel, f"/me VoteYea %s rolled %d. They won %d points for rolling above 90, now they have %d points VoteYea" % (ctx.author.name, result, delta * 3, db.get_points(ctx.author.name)))
            elif result <= 99:
                db.change_points(ctx.author.name, delta * 4)
                await ws.send_privmsg("#%s" % ctx.channel, f"/me VoteYea VoteYea %s rolled %d. They won %d points for rolling above 95, now they have %d points VoteYea VoteYea" % (ctx.author.name, result, delta * 4, db.get_points(ctx.author.name)))
            elif result == 100:
                db.change_points(ctx.author.name, delta * 10)
                await ws.send_privmsg("#%s" % ctx.channel, f"/me VoteYea VoteYea VoteYea %s rolled 100! They win a jackpot of %s points! They now have %d points VoteYea VoteYea VoteYea" % (ctx.author.name, delta * 10, db.get_points(ctx.author.name)))
...
async def command_levelup(ctx):
    ...
    if db.change_points(ctx.author.name, -cost):
        ...
        await ws.send_privmsg("#%s" % ctx.channel, f"/me %s is now level %d! VoteYea" % (ctx.author.name, cur + 1))
    else:
    ...
    if db.change_points(ctx.author.name, -amount):
        ...
        if type(result) == str:
            ...
            if ctx.channel == "twitch_plays_go_":
                await ws.send_privmsg("#%s" % ctx.channel, f"/me %s is now a channel VIP! VoteYea" % ctx.author.name)
            else:
...
async def command_song(ctx):
    ws = bot._ws
    await ws.send_privmsg("#%s" % ctx.channel, f"/me Music courtesy of Chilled Cow: https://youtube.com/c/chilledcow")
    ...
    if len(params) > 1:
        ...
        if points is None:
            ...
            if db.change_points(ctx.author.name, -amount):
                ...
                await ws.send_privmsg("#%s" % ctx.channel, f"/me You don't have that many points to give.")
...
async def command_joinstream(ctx):
    ...
    if ctx.author.is_mod:
        ...
        await ws.send_privmsg("#%s" % ctx.channel, f"/me Only moderators can use joinstream.")
...
async def command_m(ctx):
    ws = bot._ws
    await ws.send_privmsg("#%s" % ctx.channel, f"/me Just type the move, no spaces.")
...
async def command_veto(ctx):
    ...
    try:
        veto = params[0]
        processed = veto.replace("+", "").replace("#",
                                                  "").casefold().replace("x", "")
    except:
